...
inline vector<std::string> split(const std::string_view strview, const std::string& delim)
{
    vector<std::string> result;
    size_t start = 0;

    for (size_t found = strview.find(delim); found != std::string::npos; found = strview.find(delim, start))
    {
